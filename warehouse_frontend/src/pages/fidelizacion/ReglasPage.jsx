...
export default function ReglasPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res) => { toast.success(res.data.message ?? 'Regla creada'); invalidate(); closeModal() },
    onError: (err) => { applyApiErrors(err, setError); toast.error(apiErrorMessage(err)) },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => updateRegla(id, data),
    onSuccess: (res) => { toast.success(res.data.message ?? 'Regla actualizada'); invalidate(); closeModal() },
    onError: (err) => { applyApiErrors(err, setError); toast.error(apiErrorMessage(err)) },
  })

  const deleteMutation = useMutation({
    mutationFn: deleteRegla,
    onSuccess: (res) => { toast.success(res.data.message ?? 'Regla desactivada'); invalidate(); setDeleteTarget(null) },
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Editar regla' : 'Nueva regla'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Input
            ...
            {...register('puntos_por_unidad', { required: 'Requerido', min: { value: 1, message: 'Mínimo 1' } })}
          />
          ...
          <Input label="Recompensa" error={errors.recompensa?.message} {...register('recompensa', { required: 'Requerido' })} />
